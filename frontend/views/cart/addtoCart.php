...
.load {
  width: 200px;
  height: 200px;  
  background:url(https://www.cuisson.co.uk/templates/cuisson/supersize/slideshow/img/progress.BAK-FOURTH.gif) no-repeat center center; 
}
      ...
      //$sepratedprc = explode('-', $c['price']);
       ...
       echo '<div class="row cartList" id="cartitem_'.$c['vpid'].'">
        ...
        <div class="col-xs-4 col-sm-2 col-sm-push-1 productValue">
          ...
          <p class="value"><i class="fa fa-minus remove" id="cartm_'.$c['vpid'].'" style="cursor:pointer;"></i><span id="dis_'.$c['vpid'].'">'.$c['quantity'].'</span><i class="fa fa-plus add" id="cartp_'.$c['vpid'].'" style="cursor:pointer;"></i></p>              
        </div>
        <div class="col-xs-4 col-sm-2 col-sm-push-1 productValue totalCost">
          <p class="label">Total</p>
          <p class="value" >Rs.<span id="total_'.$c['vpid'].'">'.$c['total'].'</span>&nbsp;</p>             
          </div>            
          <a href="index.php?r=cart/removefromcart&vpid='.$c['vpid'].'" class="cta">Remove</a>';         
   /*    if(sizeof($address)>0)
       {                                  
...
<!--        <div class="col-xs-6 text-right">< ?=$sepratedprc[2].'&nbsp;'?><span id="grandtotal"></span></div>-->
      ...
      function getshipping(){
          $.ajax({                        
                ...
                success:  function(result) { 
                  ...
                  /*    For Currency
                   *   var ship=value['shipping'];
                    alert(ship);
                    if(ship!=0){
                     var shipval=ship.split('-');
                     //alert(shipval[0]);
                    
                       //$("#shipping_"+value['vpid']).html(shipval[0]+' '+shipval[2]);
                       $("#shipping_"+value['vpid']).html(shipval[0]);
                       //alert('hi ship val..'+shipval[0]);
                      // st= st+shipval[0];
                       var st1 = parseFloat(shipval[0]);
                       //alert('st...'+st1);
                       st=st+st1;
                      //alert(total);
                       total=value['itemtotal']+st1;
                       gt=gt+total;
                       //alert('gt..'+gt)
                    }
                    else{
                        $("#shipping_"+value['vpid']).html('-');
                        //alert(st);
                        //alert('hi m in else'+ship);
                        //alert('hi m in else st..'+st);
                        st=st+ship;
                        total= value['itemtotal']+ship;
                        gt=gt+total;
                        //alert('gt..'+gt);
                    }  */
                       //alert(st);
  ...
  .ajaxStop(function () {
   ...
   if($(this).hasClass('fa-minus'))
    ...
    else
    {
      ...
      $.ajax({
           type:"GET",
           url:"index.php?r=cart/addtocart",          
           data:{userid:<?php echo \Yii::$app->user->identity->id;?>,   
                 vpid:id_[1],
                 quantity:quant},
           success:  function(result) {
...
/******************************************************************/    
      ...
      $("#pay").click(function(){                
                ...
                {   
                     ...
                     // csrf="YmdWODR0MjkBDiF.XRpWe1IuMGdBOEtUWjBucWsNVWgTU2RCTERHSA==";
                       $.ajax({
                        type:"GET",
