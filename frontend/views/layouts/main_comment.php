...
<?php 
/*$spplit = split('=',$qrystrarr[1]);

	if($spplit[0]=='prid'){
		$spltid = (int)$spplit[1];

		$getproductname= backend\models\Product::find()->where(['prid'=>$spltid])->one();
                $proddesctag = strip_tags($getproductname['description']);
                $proddesc = substr($proddesctag, 0, 200);	 

$getproductimg= backend\models\ProductImages::find()->where(['prid'=>$spltid])->andWhere(['ismain'=>1])->one();
 $imgurl = 'https://www.digin.in/images/productimages/'.$spltid.'/'.$getproductimg['image'];
	$prodpgeurl =  'https://www.digin.in/index.php?r=search/searchproducts&prid='.$spltid;
	 ?>
          <title><?php echo $getproductname['prodname']?> | Digin - Your Market is here!</title>
         <meta property="og:type" content="product"/>
                <meta property="og:url" content="<?php echo $prodpgeurl ?>"/>
          <meta property="og:title" content="Shop for <?php echo $getproductname['prodname']?> on Digin - Your market is here !"/>
          <meta property="og:description" content="<?php echo $proddesc ?>" />
          <?php   if($getproductimg['image'] ==''){?>
                     <meta property="og:image" content="<?php echo 'https://digin.in/images/productimages/default_image.png'; ?>"/>
                     <meta name="twitter:image" content="<?php echo 'https://digin.in/images/productimages/default_image.png'; ?>"/>  
              <?php   }else{ ?>
                    <meta property="og:image" content="<?php echo $imgurl ?>"/>
                    <meta name="twitter:image" content="<?php echo $imgurl ?>" />
              <?php   }?>
	 
<meta name="twitter:title" content="Shop for <?php echo $getproductname['prodname']?> on Digin - Your market is here !" />
<meta name="twitter:description" content="<?php  echo $proddesc ?>" />
	<?php } elseif($spplit[0]=='vid'){
		$spltid = (int)$spplit[1];
		$getvendordetail= \backend\models\Vendor::find()->where(['vid'=>$spltid])->one();
                $url='https://www.digin.in/images/vendorlogo/';
$venpgeurl =  'https://www.digin.in/index.php?r=search/searchvendors&vid='.$spltid;
$address = array($getvendordetail['address1'],$getvendordetail['address2'],$getvendordetail['city'],$getvendordetail['state'],$getvendordetail['country'],$getvendordetail['pin']);

	 ?>
         <title><?php echo $getvendordetail['businessname']?> | Digin - Your Market is here! </title>
 <meta property="og:type" content="place"/>
  <meta property="og:url" content="<?php echo $venpgeurl ?>"/>
	<meta property="og:title" content="Find <?php echo $getvendordetail['businessname']?> on Digin - Your market is here !"/>
<meta property="og:description" content="<?php echo $address[0],',',$address[1],',',$address[2],',',$address[3],',',$address[4] ?>" />
 <meta property="place:location:latitude"  content="<?php echo Yii::$app->session['lat'] ?>" /> 
  <meta property="place:location:longitude" content="<?php echo Yii::$app->session['lng'] ?>" />
...
<meta name="twitter:title" content="Find <?php echo $getvendordetail['businessname']?> on Digin - Your market is here !" />
<meta name="twitter:description" content="<?php echo $address[0],',',$address[1],',',$address[2],',',$address[3],',',$address[4] ?>" />

	 <?php   if($getvendordetail['logo'] ==''){?>
			<meta property="og:image" content="<?php echo 'https://digin.in/images/vendorlogo/default_image.png'; ?>"/>
<meta name="twitter:image" content="<?php echo 'https://digin.in/images/vendorlogo/default_image.png'; ?>"/>
		<?php   }else{
              
                           $filename = '/home/ad045fcf/public_html/digin.in/advanced/frontend/web/images/vendorlogo/'.$spltid.'/'.$getvendordetail['logo'];

                  /**for get extension of image **/
  /*$imgextension = new SplFileInfo('/home/ad045fcf/public_html/digin.in/advanced/frontend/web/images/vendorlogo/'.$spltid.'/'.$getvendordetail['logo']);
                $extension = $imgextension->getExtension();

                $original_file = getimagesize($filename);

	        $original_width = $original_file[0];
		    $original_height = $original_file[1];

			if	($original_width < $original_height){
	        $imgwidth = 200;
			$new_height = ($original_height*$imgwidth)/$original_width;
			
			$newheight = round($new_height);
		 ...
		 if($extension == 'jpg'){
			...
			$thumb_width = $imgwidth;
			$thumb_height = $newheight;
			$thumb_img = imagecreatetruecolor($thumb_width, $thumb_height);
			
			
			imagecopyresized($thumb_img, $original_img,
						   0, 0,
						   0, 0,
						   $thumb_width, $thumb_height,
						   $original_width, $original_height);
		
  imagejpeg($thumb_img, '/home/ad045fcf/public_html/digin.in/advanced/frontend/web/images/vendorlogo/'.$spltid.'/'.$getvendordetail['logo']);}elseif($extension == 'png'){
	    ...
	    imagecopyresized($thumb_img, $original_img,
						   ...
						   $thumb_width, $thumb_height,
						   $original_width, $original_height);
		imagepng($thumb_img, '/home/ad045fcf/public_html/digin.in/advanced/frontend/web/images/vendorlogo/'.$spltid.'/'.$getvendordetail['logo']);}
			...
			imagedestroy($original_img);
				...
				$imgheight = 200;
				$new_width = ($original_width*$imgheight)/$original_height;
		
				$newwidth = round($new_width);
  ...
  imagejpeg($thumb_img, '/home/ad045fcf/public_html/digin.in/advanced/frontend/web/images/vendorlogo/'.$spltid.'/'.$getvendordetail['logo']);}elseif($extension == 'png'){
		...
		imagepng($thumb_img, '/home/ad045fcf/public_html/digin.in/advanced/frontend/web/images/vendorlogo/'.$spltid.'/'.$getvendordetail['logo']);}
			...
			elseif($original_height == $original_width ){
				if($original_width <= 200 || $original_height <= 200){
            if($extension == 'jpg'){
...
?>
               <meta property="og:image" content="<?php echo $url.$spltid.'/'.$getvendordetail['logo'] ?>"/>
               <meta name="twitter:image" content="<?php echo $url.$spltid.'/'.$getvendordetail['logo'] ?>" />
		
        <?php } ?>
	<?php }
else {
	echo '<title>Welcome To Digin</title>' ;
} */?>
  ...
  if (navigator.geolocation) {
    ...
    navigator.geolocation.getCurrentPosition(function(position) {
     ...
     geocoder.geocode({'location': pos}, function(results, status) {   
      country=results[2].address_components[4].long_name;
										...
										<?php $session = Yii::$app->session;?>
										   <form role="search" id="searchit" method="GET" class="" action="index.php">
											   ...
											   <input type="text" id="type" name="search" placeholder="Search for Products, Services, Categories, Brands and Vendors" class="searchInput typesearch searchinp">											
												<!-- ------------end-------------- -->
												<button id="searchsubmit" type="submit" class="searchButton searchbtn" value="Search"><img src="../web/images/SearchLens.png" class="serchicn"></button>
											</form>
									</span>	
									<img id="locicon" src="images/loc.png" class="imgloc locatn" >
								</div>	
								<div style="display: none; z-index:10; position:absolute; margin-left:175px;" class="loc">
