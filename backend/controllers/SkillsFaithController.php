...
class SkillsFaithController extends Controller
{
    ...
    protected function findModel($id)
    {
      ...
      public function actionApprovedskillfaith()
	{
        $vid=$_POST['id'];
        $activ=$_POST['idapprov'];
        //echo $vid."..".$activ;
        if($activ==0){
        $vidactiv= \frontend\models\SkillsFaith::updateAll(['Is_approved'=>1], 'faithid='.$vid);
        }else{
        $vidactiv= \frontend\models\SkillsFaith::updateAll(['Is_approved'=>0], 'faithid='.$vid); 
        }
         if($vidactiv>0)
            echo "1";
        else{
