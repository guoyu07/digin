...
class VendorCurrencySettingController extends Controller
{
    ...
    public function saveModel(array $modelarray)
    {
       $model = $modelarray['model'];
        //$modelvendor = $modelarray['mdlvendor'];
        $vid = $_GET['id'];
        $uid = \yii::$app->user->identity->id;
        $venid = Vendor::find()->where(['user_id'=>$uid])->one();
      ...
      $vencursts = VendorCurrencySetting::findAll(['vid'=>$vid]);
      
      foreach ($vencursts as $ven){
       $result = $model->applyCurrencySetting($ven['country'],$ven['currency'],$ven['currency_rate'],$ven['percentaddition']);
         //var_dump($ven);
      }
      if($succs) 
            {
