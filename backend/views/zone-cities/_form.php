...
<div class="zone-cities-form">

    <?php $form = ActiveForm::begin(); ?>

    <?php $zones= backend\models\Zone::find()->all();
          $zonedata=  ArrayHelper::map($zones, 'zid', 'name')?>
     <div class="row">
        <div class="col-xs-3">
            <?= $form->field($model, 'zid')->dropDownList($zonedata,['prompt'=>'Select']) ?>
        </div>
    </div>
...
<script type="text/javascript">
     $(document).ready(function(){
         $('#zonecities-state').change(function(){            
            var state=$('#zonecities-state').val();
            alert(state);
             $.ajax({
                type:"POST",
                   url:"index.php?r=zone-cities/getcity",           
                   data:{stateid:state},
